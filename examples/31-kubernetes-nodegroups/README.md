...
Creates an **OpenStack Magnum** Kubernetes cluster and an **additional node group** (`openstack_containerinfra_nodegroup_v1`) with its own image and flavor—similar to `openstack coe cluster create` plus `openstack coe nodegroup create`. The default cluster template name is `Kubernetes v1.34.4`; change `cluster_template_name` if your cloud uses a different label.
...
```bash
openstack coe cluster create tf-lesson-k8s-nodegroups \
  --cluster-template "Kubernetes v1.34.4" \
  --master-count 1 --node-count 1 \
  --keypair <your-keypair> \
  --master-flavor m.2vcpu.16gb \
  --floating-ip-enabled

openstack coe nodegroup create --node-count 1 \
  --image "CAPI v1.34.4 x86_64" --flavor g.8vcpu.32gb \
  tf-lesson-k8s-nodegroups big
```
...
2. From **`examples/31-kubernetes-nodegroups`**: `terraform init` / `plan` / `apply`.
